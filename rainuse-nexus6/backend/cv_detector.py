"""
cv_detector.py — AquaVista CV Pipeline (Corrected)
====================================================

WHAT COOLING TOWERS ACTUALLY LOOK LIKE FROM SATELLITE
------------------------------------------------------
Source: Direct observation of satellite imagery + Lancet Digital Health 2024
(Wong et al., "Automated cooling tower detection through deep learning for
Legionnaires' disease outbreak investigations")

From above, cooling towers appear as:
  1. DARK rectangular clusters — dark grey to near-black
  2. DARKER than surrounding roof membrane (which is light grey/white/tan)
  3. Multiple small circular fan tops arranged in a 2×N grid inside the rectangle
  4. Often clustered — multiple units side by side
  5. Typical size: each fan circle ~1–5m diameter; full cluster 3–20m wide

WHAT THEY DO NOT LOOK LIKE (what my previous version incorrectly assumed):
  × Single circular "donut" — bright ring, dark centre
  × Light grey/white structure against dark background
  × Simple Hough circle targets

CORRECT DETECTION APPROACH
---------------------------
Based on the Lancet paper (Wong et al. 2024):

  Stage 1 — Object detection (sensitive):   YOLOv5 / YOLO11 fine-tuned
  Stage 2 — Classification (high PPV):      EfficientNet-b5 or equivalent

The paper achieved 95.1% sensitivity / 90.1% PPV using this two-stage pipeline
trained on 2,051 images with 7,292 labelled cooling towers from NYC + Philadelphia.

Pre-trained weights are available at: https://github.com/TowerScout/TowerScout

IMPLEMENTATION STRATEGY FOR THIS PROTOTYPE
-------------------------------------------
Priority 1: TowerScout weights — purpose-built, peer-reviewed, 95% accuracy
Priority 2: Fine-tuned YOLO on available cooling tower data
Priority 3: Classical CV targeting the CORRECT visual features:
            - Dark rectangular blobs on lighter rooftops
            - Internal texture: grid of small circles (fan tops)
            - Contrast: tower darker than surrounding roof membrane
"""
...
# ── TowerScout model (pre-trained, purpose-built for cooling towers) ───────
# Source: Wong et al., Lancet Digital Health 2024
# GitHub: https://github.com/TowerScout/TowerScout
# The TowerScout YOLOv5 was trained on 7,292 labelled cooling towers.
# It understands that cooling towers are dark rectangular clusters with fan grids.
# Download the weights file from the GitHub releases and place in this directory.
_yolo_model = None
_yolo_source = "none"

TOWERSCOUT_WEIGHTS = os.path.join(os.path.dirname(__file__), "towerscout.pt")
YOLO_WEIGHTS_SEARCH = [
    TOWERSCOUT_WEIGHTS,   # TowerScout purpose-built model (best)
    "yolo11s.pt",          # Generic YOLO — will be used with correct visual filters
    "yolov8s.pt",
    "yolov8n.pt",
]

if YOLO_OK:
    for w in YOLO_WEIGHTS_SEARCH:
        try:
            _yolo_model = _YOLO_CLS(w)
            _yolo_source = os.path.basename(w)
            is_towerscout = "towerscout" in w.lower()
            print(f"[CV] YOLO loaded: {_yolo_source}"
                  f"{'  ← PURPOSE-BUILT FOR COOLING TOWERS (best)' if is_towerscout else '  ← generic weights, will use visual filters'}")
            break
        except Exception:
            continue
...
def get_osm_footprint(lat, lng):
    ...
    try:
        r = requests.get("https://overpass-api.de/api/interpreter",
                         params={"data": query}, headers=_HDR, timeout=12)
        r.raise_for_status()
        ...
        if SHAPELY_OK:
            ...
            for i in range(len(lats)):
                j = (i + 1) % len(lats)
                a += (lons[j]-lons[i]) * (2 + math.sin(lats[i]) + math.sin(lats[j]))
...
def detect_roof(img_pil, building, bounds):
    ...
    if not coords:
        return {"detected": False, "area_sqft": 0, "confidence": 0.0,
                "over_100k": False, "method": "no_osm_footprint"}
    ...
    if not CV2_OK:
        return {"detected": True, "bbox_px": bbox, "polygon_px": poly_px,
                "area_sqft": round(area), "confidence": 0.72,
                "over_100k": area >= 100_000, "method": "osm_prior_only"}
    ...
    if crop.size == 0:
        return {"detected": True, "bbox_px": bbox, "polygon_px": poly_px,
                "area_sqft": round(area), "confidence": 0.70,
                "over_100k": area >= 100_000, "method": "osm_prior_crop_empty"}
    ...
    mask = cv2.morphologyEx(cv2.morphologyEx(mask, cv2.MORPH_CLOSE, k, iterations=2),
    ...
    if best_cnt is None:
        return {"detected": True, "bbox_px": bbox, "polygon_px": poly_px,
                "area_sqft": round(area), "confidence": 0.74,
                "over_100k": area >= 100_000, "method": "osm_prior_cv_no_contour"}
    ...
    overlap = np.count_nonzero(cv2.bitwise_and(prior_mask, full_m)) / max(np.count_nonzero(full_m), 1)
    conf = min(0.95, 0.55 + 0.30*overlap + 0.10*ratio)
    ys2, xs2 = np.where(full_m > 0)
    rbbox = [int(xs2.min()), int(ys2.min()), int(xs2.max()), int(ys2.max())] if len(xs2) else bbox

    return {"detected": True, "bbox_px": rbbox, "polygon_px": poly_px,
            "area_sqft": round(refined_area), "confidence": round(conf, 3),
            "over_100k": refined_area >= 100_000,
            "method": "osm_prior_plus_cv_refinement"}


# ══════════════════════════════════════════════════════════════════════════════
# COOLING TOWER DETECTION — CORRECT APPROACH
# Based on: Wong et al., Lancet Digital Health 2024
# Visual ground truth: satellite image shows dark rectangular fan-grid clusters
# ══════════════════════════════════════════════════════════════════════════════
...
def _dark_cluster_score(gray_patch):
    """
    Score how dark and uniform this patch is — cooling towers are the darkest
    rectangular features on a commercial rooftop.

    From the satellite image:
    - Rooftop membrane: ~160-220 grey level (light)
    - Cooling tower cluster: ~50-120 grey level (dark)
    - Contrast between tower and roof is the primary visual cue

    Returns score in [0, 1]: higher = more tower-like (darker than surround)
    """
    if patch.size == 0 if (patch := gray_patch).size == 0 else False:
        return 0.0
    mean_val = float(gray_patch.mean())
    # Score: dark patches (mean < 130) score higher
    # Typical roof: 160-220; typical tower: 60-120
...
def _fan_grid_score(gray_patch, min_circles=2):
    """
    Score the presence of a circular fan-top grid pattern within a dark region.

    From the satellite image, cooling towers show:
    - Multiple circular fan openings (typically 2-6 per unit, arranged in grid)
    - Each fan circle is slightly lighter than the surrounding dark casing
    - The grid creates a repeating circular texture

    This validates that a dark rectangular region is specifically a cooling tower
    (rather than a rooftop shadow, HVAC equipment, or dark roof material).
    """
    ...
    # Enhance local contrast to make fan circles visible
    patch_eq = cv2.equalizeHist(gray_patch)

    # Look for small circles (fan tops) within the dark cluster
    # Fan diameter: typically 0.5-2m = 1-6 px at zoom 17-18
    ...
    # Score based on finding multiple fan circles (≥2 = likely tower cluster)
...
def _detect_dark_rectangles(gray, enhanced_gray, search_mask, zoom):
    """
    PRIMARY DETECTOR for cooling towers.

    Visual principle (from satellite image observation):
    - Cooling towers are the DARKEST rectangular structures on rooftops
    - They are dark grey to near-black relative to the light roof membrane
    - They form compact rectangular or square clusters
    - Size: ~5-30 metres = 4-25 px at zoom 17, 8-50 px at zoom 18

    Detection strategy:
    1. Threshold to find dark regions (towers darker than roof)
    2. Morphological ops to clean up and connect tower fan openings
    3. Find rectangular contours of the right size
    4. Score each candidate by darkness + fan-grid texture
    """
    mpp = ZOOM_MPP.get(zoom, 1.19)
    # Tower cluster size range in pixels
    min_side_px = max(6,  int(4.0  / mpp))   # 4m minimum
    max_side_px = min(400, int(80.0 / mpp))   # 80m maximum cluster (groups of towers)
    ...
    # Method A: Threshold for dark regions
    # Cooling tower range: grey level ~50-120 (vs roof ~160-220)
    # Use adaptive threshold to handle variable lighting across tiles
    dark_abs = cv2.inRange(gray_search, 0, 125)   # absolute dark
    dark_adapt = cv2.adaptiveThreshold(
        gray_search, 255,
        cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV,
        blockSize=51, C=15
    )
    # Combine: pixels that are dark AND darker than their local neighbourhood
    dark_combined = cv2.bitwise_and(dark_abs, dark_adapt)

    # Morphological closing: connect individual fan openings into one cluster blob
    ...
    # Find contours = candidate tower clusters
    ...
    for c in cnts:
        ...
        if bw > max_side_px * 1.5 or bh > max_side_px * 1.5:
            continue
        # Aspect ratio: towers are roughly square to 1:3 rectangles
        aspect = bw / max(bh, 1)
        ...
        # Score 1: Darkness (cooling towers are dark)
        patch = gray[y:y+bh, x:x+bw]
        if patch.size == 0:
            continue
        darkness = max(0.0, min(1.0, (140.0 - float(patch.mean())) / 90.0))
        if darkness < 0.1:  # Not dark enough — skip
            continue

        # Score 2: Fan grid texture (optional — validates dark blob is a tower)
        fan_score, n_fans = _fan_grid_score(patch)

        # Score 3: Compactness (towers are compact rectangles, not thin lines)
        compactness = min(aspect, 1.0/aspect)  # 1.0 = square, lower = elongated

        # Composite confidence
        conf = (
            0.50 * darkness        +  # primary cue: tower is dark
            0.25 * fan_score       +  # secondary: internal fan grid
            0.15 * compactness     +  # shape: compact rectangle
            0.10 * min(1.0, area_px / (min_side_px**2 * 4))  # size adequacy
        )
...
def _yolo_detector(img_pil, offset_x, offset_y, is_towerscout=False):
    """
    YOLO detection with correct post-filtering.

    If TowerScout weights: use directly — it knows what cooling towers look like
    If generic YOLO:       apply dark-cluster validation to filter detections
                           Generic YOLO will fire on rooftop equipment generically
                           but we keep only detections that are also DARK patches
    """
    if _yolo_model is None:
        return []

    img_np   = np.array(img_pil)
    gray     = cv2.cvtColor(img_np, cv2.COLOR_RGB2GRAY) if CV2_OK else None
    ...
    for r in results:
        ...
        for box in r.boxes:
            ...
            if is_towerscout:
                # TowerScout already trained for this — trust the detection
                conf = min(0.92, conf_yolo)
                detector_tag = "towerscout_yolo"
            else:
                # Generic YOLO — must pass the dark-cluster validation
                if gray is not None and 0<=by0<by1<=gray.shape[0] and 0<=bx0<bx1<=gray.shape[1]:
                    patch = gray[by0:by1, bx0:bx1]
                    darkness = max(0.0, min(1.0, (140.0-float(patch.mean()))/90.0))
                    if darkness < 0.12:
                        continue  # Not dark — not a cooling tower
                    fan_sc, _ = _fan_grid_score(patch)
                    conf = min(0.80, conf_yolo * 0.40 + darkness * 0.40 + fan_sc * 0.20)
                else:
                    conf = conf_yolo * 0.35  # No validation possible
            ...
            candidates.append({
                "cx": cx_c+offset_x, "cy": cy_c+offset_y, "r": r_eq,
...
def _iou(a, b):
    """IoU for bounding boxes [x0,y0,x1,y1]."""
...
def _deduplicate(candidates, iou_thresh=0.40):
    """Keep highest-confidence candidate when multiple detections overlap."""
    kept = []
...
def detect_towers(building, roof_data):
    """
    Multi-stage cooling tower detection.

    Stage 1 (Primary):
      Dark-rectangular-cluster detector:
        - Thresholds for dark regions (towers are DARKER than roof)
        - Morphological ops to merge individual fan openings
        - Fan-grid texture validator (circular fans inside dark rectangle)

    Stage 2 (Secondary):
      YOLO-based detector:
        - TowerScout weights: direct high-confidence detections
        - Generic YOLO: dark-cluster validated to reduce false positives

    Fusion: merge all candidates, deduplicate by IoU, rank by confidence.

    Reference: Wong et al. (2024), Lancet Digital Health — two-stage
    YOLOv5 + EfficientNet-b5 achieves 95.1% sensitivity on NYC/Philadelphia.
    TowerScout weights at: github.com/TowerScout/TowerScout
    """
    if not roof_data.get("detected"):
        return {"present": False, "count": 0, "towers": [], "confidence": 0.0,
    ...
    # Fetch best zoom
    img_pil, bounds, zoom, sharpness = _fetch_best_zoom(
        building["lat"], building["lng"], zooms=(17, 18, 19), radius=1
    )
    img_np = np.array(img_pil)
    gray   = cv2.cvtColor(img_np, cv2.COLOR_RGB2GRAY)
    w, h   = img_pil.size

    # Build context mask — only search within roof + buffer
    ...
    # Stage 1: Dark rectangle detector
    ...
    deduped = deduped[:20]  # Max 20 towers per frame

    print(f"  [CT] Final: {len(deduped)} towers (dedup+thresh)")

    if not deduped:
        return {"present": False, "count": 0, "towers": [],
                "confidence": 0.0,
                "method": "no_dark_clusters_found_on_roof",
                "note": "If building has cooling towers, try TowerScout weights: github.com/TowerScout/TowerScout",
                "zoom_used": zoom, "sharpness": round(sharpness,1)}
    ...
    return {
        ...
        "model_note": (
            "For best results (95%+ accuracy), use TowerScout weights: "
            "github.com/TowerScout/TowerScout  "
            "[Wong et al., Lancet Digital Health 2024]"
        ),
    }
...
def annotate(img_pil, roof, towers, building):
    ...
    if roof.get("bbox_px"):
        ...
        lbl = (f"ROOF  {roof.get('area_sqft',0):,} ft²  "
    ...
    # Tower bounding boxes (amber rectangles — matching actual tower shape)
...
def _save_images(clean, annotated, building):
    base = os.path.join(os.path.dirname(__file__), "saved_cv_images",
...
def run_pipeline(building):
    zoom = building.get("zoom", 17)
    print(f"\n[CV] {building.get('name','?')}")
    print(f"     GPS: {building['lat']:.5f}N  {abs(building['lng']):.5f}W  zoom={zoom}")

    try:
        img_roof, bounds_roof = fetch_tiles(building["lat"], building["lng"],
                                            zoom=zoom, radius=1)
    ...
    return {
        ...
        "cv_model": {
            ...
            "towerscout_note": "github.com/TowerScout/TowerScout — Wong et al., Lancet 2024",
        },
        "roof":                  roof,
